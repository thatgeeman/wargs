...
from .prompts import HypothesisAgPrompt, ResearchPlannerAgPrompt, ResearchTaskAgPrompt
from .schemas import (
...
class Agent(ABC):
    def __init__(self, name, input, output_schema, tools: list = []):
        self.name = name
        self.input = input
    ...
    def run(self, to_json=False):
        ...
        return (
            ...
            if not to_json
            else self.output_schema.model_validate(
                json.loads(choice.message.content.strip())
            )
        )
...
class HypothesisAgent(Agent):
    def __init__(self, input):
        ...
        super().__init__(self.name, input, self.output_schema)


class ResearchPlanner(Agent):
    def __init__(self, input: str, hypothesis: ManyHypothesesAgSchema = []):
        self.instance_id = uuid.uuid4()
...
class ResearchTask(Agent):
    def __init__(
        self, input: str, hypothesis: ManyResearchTaskAgSchema = [], tools: list = []
    ):
        self.instance_id = uuid.uuid4()
        self.name = "ResearchTaskAgent_" + str(self.instance_id)
        self.tools = tools
        self.prompts = ResearchTaskAgPrompt(
            question=input, hypothesis=hypothesis, tools=tools
        )
        self.output_schema = ManyResearchTaskAgSchema()
        super().__init__(self.name, input, self.output_schema, self.tools)
